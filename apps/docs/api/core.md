...
Creates an Options Store factory. Options include `state`, `getters`, `actions`, `persist`, and `debug`. Store state is tracked by Sveltinia and exposed through normal store properties.
