...
import { createSvelteKitSveltinia } from '../src/adapters/sveltekit.js'

describe('SvelteKit integration', () => {
  it('serializes and hydrates request scoped state', () => {
    const api = createSvelteKitSveltinia(); const one = api.create(); const useCounter = defineStore('counter', { state: () => ({ count: 0 }) }); useCounter(one).count = 4
