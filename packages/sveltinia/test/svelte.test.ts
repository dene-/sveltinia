...
import { createSveltinia, defineStore, getActiveSveltinia, setActiveSveltinia } from '../src/index.js'
...
describe('SvelteKit integration', () => {
  ...
  it('does not activate request roots globally', () => {
    setActiveSveltinia(undefined)
    const api = createSvelteKitRoot(); api.create()
    expect(getActiveSveltinia()).toBeUndefined()
  })
})
